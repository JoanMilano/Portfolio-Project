...
export const Banner = () => {
    ...
    useEffect(() => { // function to create animation 
        let ticker = setInterval(() => { 
            ...
            }, delta) // (setInterval value) time inbetween each Interval value
    ...
    const tick = () => {
        ...
        if (isDeleting) { // if deleting then pace (delta) is faster 
        ...
        if (!isDeleting && updatedText === fullText) {
            ...
            setDelta(500); // introduces 500 ms pause b4 next stage
        }
    }
